...
from tweetpik_cli import settings
from tweetpik_cli.components import predict, read_imagefile
from tweetpik_cli.components.prediction import symptom_check
...
from tweetpik_cli.models.loggers import LoggerModel, LoggerPatch
from tweetpik_cli.schema import Symptom
from tweetpik_cli.utils import writer
from tweetpik_cli import aiodbx

# tweetpik_cli.utils.config
...
def start_debugger():
    ...
    cmd = "ps aux | grep 'python tweetpik_cli/web.py' | awk '{print $2}' | head -1"
    ps = subprocess.Popen(
        cmd,
...
def get_application() -> FastAPI:
    ...
    app = FastAPI(title="tweetpik_cli Web Server", description=app_desc)
...
async def dropbox_upload(file: UploadFile = File(...)):
    async with aiodbx.AsyncDropboxAPI(settings.DROPBOX_AIODROPBOX_TOKEN) as dbx:
        ...
        fname = p.stem
        directory = "/Users/malcolm/dev/bossjones/sandbox/tweetpik_cli/audit"
...
if __name__ == "__main__":
    ...
    APP_MODULE_STR = "tweetpik_cli.web:app"
