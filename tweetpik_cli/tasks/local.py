...
def serve(ctx, loc="local", verbose=0, cleanup=False):
    ...
    _cmd = r"""
pkill -f "tweetpik_cli/web.py" || true
pgrep -f "tweetpik_cli/web.py" || true
    """
    ...
    ctx.run("python tweetpik_cli/web.py")
...
def pstree(ctx, loc="local", verbose=0, cleanup=False):
    ...
    # lsof -nP +c 15 | grep LISTEN
    _cmd = r"""
ps aux | grep 'python tweetpik_cli/web.py' | awk '{print $2}' | head -1
    """
