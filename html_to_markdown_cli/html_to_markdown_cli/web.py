...
from html_to_markdown_cli import settings
from html_to_markdown_cli.components import predict, read_imagefile
from html_to_markdown_cli.components.prediction import symptom_check
from html_to_markdown_cli.dbx_logger import (  # noqa: E402
    generate_tree,
    get_lm_from_tree,
    get_logger,
    intercept_all_loggers,
)
from html_to_markdown_cli.models.loggers import LoggerModel, LoggerPatch
from html_to_markdown_cli.schema import Symptom
from html_to_markdown_cli.utils import writer
from html_to_markdown_cli import aiodbx

# html_to_markdown_cli.utils.config
...
def start_debugger():
    ...
    cmd = "ps aux | grep 'python html_to_markdown_cli/web.py' | awk '{print $2}' | head -1"
    ps = subprocess.Popen(
        cmd,
...
def get_application() -> FastAPI:
    ...
    app = FastAPI(title="html_to_markdown_cli Web Server", description=app_desc)
...
async def dropbox_upload(file: UploadFile = File(...)):
    async with aiodbx.AsyncDropboxAPI(settings.DROPBOX_AIODROPBOX_TOKEN) as dbx:
        ...
        fname = p.stem
        directory = "/Users/malcolm/dev/bossjones/sandbox/html_to_markdown_cli/audit"
...
if __name__ == "__main__":
    ...
    APP_MODULE_STR = "html_to_markdown_cli.web:app"
