...
# from html_to_markdown_cli.utils.parser import get_domain_from_fqdn
...
def pylint(
    ...
    """
    pylint html_to_markdown_cli folder
    Usage: inv ci.pylint
    """
    env = get_compose_env(ctx, loc=loc)
    ...
    if tests:
        ...
        ctx.run(
            "pylint --output-format=colorized --rcfile ./lint-configs-python/python/pylintrc tests html_to_markdown_cli"
        )
    elif specific:
        ctx.run(
            f"pylint --output-format=colorized --disable=all --enable={specific} --rcfile ./lint-configs-python/python/pylintrc tests html_to_markdown_cli"
        )
    elif error_only:
        ctx.run(
            f"pylint --output-format=colorized --disable=all --enable=F,E --rcfile ./lint-configs-python/python/pylintrc tests html_to_markdown_cli"
        )
    else:
        ctx.run(
            "pylint --output-format=colorized --disable=all --enable=F,E --rcfile ./lint-configs-python/python/pylintrc html_to_markdown_cli"
        )
...
def mypy(ctx, loc="local", verbose=0):
    """
    mypy html_to_markdown_cli folder
    Usage: inv ci.mypy
    """
    env = get_compose_env(ctx, loc=loc)
    ...
    # ctx.run("mypy --config-file ./lint-configs-python/python/mypy.ini html_to_markdown_cli tests")
    ctx.run("mypy --config-file ./setup.cfg html_to_markdown_cli tests")
...
def isort(ctx, loc="local", check=False, dry_run=False, verbose=0, diff=False):
    """
    isort html_to_markdown_cli module. Some of the arguments were taken from the starlette contrib scripts. Tries to align w/ black to prevent having to reformat multiple times.
    ...
    _cmd += " html_to_markdown_cli tests"
...
def pre_start(ctx, loc="local", check=True, debug=False):
    """
    pre_start html_to_markdown_cli module
    """
    env = get_compose_env(ctx, loc=loc)
    ...
    # ctx.run("python html_to_markdown_cli/api/tests_pre_start.py")
...
def pytest(
    ...
    _cmd += r" --cov-config=setup.cfg --verbose --cov-append --cov-report=term-missing --cov-report=xml:cov.xml --cov-report=html:htmlcov --cov-report=annotate:cov_annotate  --showlocals --tb=short --cov=html_to_markdown_cli tests"
