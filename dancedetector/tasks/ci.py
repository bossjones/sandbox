...
def pylint(ctx, loc="local", tests=False, everything=False, specific=""):
    ...
    if tests:
        ...
        ctx.run("pylint --output-format=colorized --rcfile ./lint-configs-python/python/pylintrc tests dancedetector")
    elif specific:
        ctx.run(
            f"pylint --output-format=colorized --disable=all --enable={specific} --rcfile ./lint-configs-python/python/pylintrc tests dancedetector"
        )
    else:
        ctx.run(
            "pylint --output-format=colorized --disable=all --enable=F,E --rcfile ./lint-configs-python/python/pylintrc dancedetector"
        )
...
def mypy(ctx, loc="local", verbose=0):
    ...
    # ctx.run("mypy --config-file ./lint-configs-python/python/mypy.ini dancedetector tests")
    ctx.run("mypy --config-file ./setup.cfg dancedetector tests")
...
def isort(ctx, loc="local", check=False, dry_run=False, verbose=0, diff=False):
    """
    isort dancedetector module. Some of the arguments were taken from the starlette contrib scripts. Tries to align w/ black to prevent having to reformat multiple times.
...
def bandit(ctx, loc="local", package=True, tests=True, verbose=0):
    """
    bandit dancedetector module. Some of the arguments were taken from the starlette contrib scripts. Tries to align w/ black to prevent having to reformat multiple times.
...
def pre_start(ctx, loc="local", check=True, debug=False):
    """
    pre_start dancedetector module
    """
    env = get_compose_env(ctx, loc=loc)
    ...
    # ctx.run("python dancedetector/api/tests_pre_start.py")
...
def pytest(
    ...
    _cmd += r" --cov-config=setup.cfg --verbose --cov-append --cov-report=term-missing --cov-report=xml:cov.xml --cov-report=html:htmlcov --cov-report=annotate:cov_annotate  --showlocals --tb=short --cov=dancedetector tests"
