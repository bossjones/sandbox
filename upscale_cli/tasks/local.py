...
def serve(ctx, loc="local", verbose=0, cleanup=False):
    ...
    _cmd = r"""
pkill -f "upscale_cli/web.py" || true
pgrep -f "upscale_cli/web.py" || true
    """
    ...
    ctx.run("python upscale_cli/web.py")
...
def pstree(ctx, loc="local", verbose=0, cleanup=False):
    ...
    # lsof -nP +c 15 | grep LISTEN
    _cmd = r"""
ps aux | grep 'python upscale_cli/web.py' | awk '{print $2}' | head -1
    """
