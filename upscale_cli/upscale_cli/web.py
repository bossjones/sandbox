...
from upscale_cli import settings
from upscale_cli.components import predict, read_imagefile
from upscale_cli.components.prediction import symptom_check
...
from upscale_cli.models.loggers import LoggerModel, LoggerPatch
from upscale_cli.schema import Symptom
from upscale_cli.utils import writer
from upscale_cli import aiodbx

# upscale_cli.utils.config
...
def start_debugger():
    ...
    cmd = "ps aux | grep 'python upscale_cli/web.py' | awk '{print $2}' | head -1"
    ps = subprocess.Popen(
        cmd,
...
def get_application() -> FastAPI:
    ...
    app = FastAPI(title="upscale_cli Web Server", description=app_desc)
...
async def dropbox_upload(file: UploadFile = File(...)):
    async with aiodbx.AsyncDropboxAPI(settings.DROPBOX_AIODROPBOX_TOKEN) as dbx:
        ...
        fname = p.stem
        directory = "/Users/malcolm/dev/bossjones/sandbox/upscale_cli/audit"
...
if __name__ == "__main__":
    ...
    APP_MODULE_STR = "upscale_cli.web:app"
